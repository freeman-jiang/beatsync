...
export class StateManager {
  ...
  static async backupState(): Promise<void> {
    try {
      ...
      // TODO: Implement room iteration when globalManager is available
      // globalManager.forEachRoom((room: any, roomId: any) => {
  ...
  static async restoreState(): Promise<boolean> {
    try {
      ...
      for (const [roomId, roomData] of Object.entries(rooms)) {
        // TODO: Implement room restoration when globalManager is available
        // const room = globalManager.getOrCreateRoom(roomId);

        // Restore audio sources
        roomData.audioSources.forEach((source) => {
          // room.addAudioSource(source);
          console.log(`Would restore audio source: ${source.title}`);
        });
