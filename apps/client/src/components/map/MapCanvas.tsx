...
export const MapCanvas = ({ canMutate }: MapCanvasProps) => {
  ...
  useEffect(() => {
    ...
    for (const state of shapes.values()) {
      seen.add(state.shape.id);
      let layer = shapeLayersRef.current.get(state.shape.id);
      const coords = state.shape.coordinates as unknown;
      ...
      if (!layer) {
        ...
        shapeLayersRef.current.set(state.shape.id, layer);
      ...
      // Bind a tooltip showing the playlist length and playback state.
      const isPlaying = state.playbackState.type === "playing";
      layer.bindTooltip(
        `<div class="text-xs"><strong>${state.shape.id.slice(0, 6)}</strong>` +
          ` · ${state.playlist.length} track(s) · ${isPlaying ? "▶" : "❚❚"}</div>`,
