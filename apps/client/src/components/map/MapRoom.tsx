...
export const MapRoom = ({ roomId }: MapRoomProps) => {
  ...
  useEffect(() => {
    ...
    for (const state of shapes.values()) {
      const gain = proximityGainForShape(ownPosition, state.shape);
      nextGains.set(state.shape.id, gain);
      mapAudio.setProximityGain(state.shape.id, gain);
    }
