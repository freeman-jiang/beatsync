...
function SortableItem(props) {
  ...
  const { sourceState, index, canMutate } = props;
            ...
            <SortableContext items={audioSources.map((src) => src.source.url)} strategy={verticalListSortingStrategy} >
              <AnimatePresence initial={true}>
                ...
                {audioSources.map((sourceState, index) => {
                  return (
                    <SortableItem key={sourceState.source.url} id={sourceState.source.url} sourceState={sourceState} index={index} canMutate={canMutate} />
                  );
                })}
